...
(function(){
 ...
 document.getElementById('capAcc').innerHTML=
   ...
   hd('Cosa dicono i numeri.')+' Il modello arriva a '+M.map(m=>`<b>${m.name}</b> ${pct(Math.max(...roles.map(r=>(m.acc[r]||{}).intact||0)))}`).join(', ')+`, contro il mapper (`+M.map(m=>`${pct(m.mapper_acc)}`).join(' / ')+'). <b>Attenzione al riferimento del mapper</b>: quel numero viene dal mapper costruito SENZA le etichette italiane ufficiali PRO-CTCAE, che su Colab non erano disponibili perché escluse dal repository. Con le etichette ufficiali lo stesso mapper, sugli stessi identici item, arriva al <b>61,4%</b>, e a quel punto <b>nessun modello lo batte</b>: il migliore si ferma al 50%. Non è un problema per la tesi, che non riguarda chi codifica meglio, ma va detto per primi. Rimuovere l\'emotività lascia l\'accuratezza quasi identica ('+M.map(m=>`${m.name} ${pct(m.ablation.term_acc_intact)}→${pct(m.ablation.term_acc_ablated)}`).join('; ')+'). '+
