...
(function(){
 ...
 document.getElementById('capAcc').innerHTML=
   ...
   hd('Cosa dicono i numeri.')+' Il modello arriva a '+M.map(m=>`<b>${m.name}</b> ${pct(Math.max(...roles.map(r=>(m.acc[r]||{}).intact||0)))}`).join(', ')+`, contro il mapper (`+M.map(m=>`${pct(m.mapper_acc)}`).join(' / ')+'). '+(Math.max(...M.map(m=>Math.max(...roles.map(r=>(m.acc[r]||{}).intact||0)))) > Math.max(...M.map(m=>m.mapper_acc||0)) ? 'Il modello <b>batte</b> il sistema che guarda solo le parole. ' : '<b>Nessun modello batte il mapper</b>: il sistema che guarda solo le parole resta davanti. Non è un problema per la tesi, che non riguarda chi codifica meglio, ma va detto per primi. ')+'<b>Il riferimento del mapper dipende dalle etichette ufficiali PRO-CTCAE italiane</b>, che sono escluse dal repository per licenza: senza, lo stesso mapper sugli stessi item perde una ventina di punti, e i confronti pubblicati prima di questa correzione usavano il numero degradato. Rimuovere l\'emotività lascia l\'accuratezza quasi identica ('+M.map(m=>`${m.name} ${pct(m.ablation.term_acc_intact)}→${pct(m.ablation.term_acc_ablated)}`).join('; ')+'). '+
